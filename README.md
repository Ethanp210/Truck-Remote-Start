...
- A physical or simulated iOS device running iOS 16 or later.
...
- **`Info.plist`** — Already referenced by the project; keep it at the repository root. It defines the URL scheme (`truckremote`) and location usage string.
...
2. OAuth uses `ASWebAuthenticationSession` with the custom URL scheme. Make sure the backend callback uses `truckremote://` unless you change it in Developer Settings.

## Developer Settings at Runtime
The Developer Settings tab lets you override `Base URL`, `Client ID`, and `Redirect Scheme` without rebuilding. Tap **Apply & Restart Auth** after edits to relaunch the PKCE flow.
